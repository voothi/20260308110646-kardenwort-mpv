...
local CYRILLIC_UPPER =
    utf8_to_table("АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯÄÖÜẞ")
...
local WORD_CHAR_MAP = {}
for _, ch in ipairs(CYRILLIC_UPPER) do
    WORD_CHAR_MAP[ch] = true
end
for _, ch in ipairs(CYRILLIC_LOWER) do
    WORD_CHAR_MAP[ch] = true
end
...
local function is_abbrev(w, lookahead)
    ...
    if _opts.anki_abbrev_smart then
        ...
        if not heuristic_suppressed and w:match("^%l+%.$") and #w <= 5 then
            return true
        end
        if w:match("^%u%.$") then
            return true
...
local function build_word_list_internal(text, keep_spaces)
    ...
    while i <= n do
        local c = chars[i]
        local token =
            { text = "", is_word = false, logical_idx = nil, visual_idx = curr_visual_idx }

        -- 1. Handle ASS Tags (Atomize)
        if c == "{" then
            local start = i
            while i <= n and chars[i] ~= "}" do
                i = i + 1
            end
        ...
        elseif c:match("^%s$") or c == "\194\160" then
            local start = i
            while i <= n and (chars[i]:match("^%s$") or chars[i] == "\194\160") do
                i = i + 1
            end
            if keep_spaces then
        ...
        elseif is_word_char(c) then
            local start = i
            while i <= n and is_word_char(chars[i]) do
                i = i + 1
            end
            token.text = table.concat(chars, "", start, i - 1)
            token.is_word = true
        ...
        elseif
            c == "\\"
            and i < n
            and (chars[i + 1] == "N" or chars[i + 1] == "n" or chars[i + 1] == "h")
        then
