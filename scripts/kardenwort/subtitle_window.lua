...
local DW_TOOLTIP_DRAW_CACHE =
    { target_idx = -1, osd_y = -1, version = -1, cl = -1, cw = -1, av = -1 }
...
function M.build_profiles()
    ...
    local shared_keys = {
        ...
        "pri_highlight_color",
        "sec_highlight_color",
        "pri_ctrl_select_color",
        "sec_ctrl_select_color",
        "pri_highlight_bold",
        "sec_highlight_bold",
    }
...
local function format_sub_wrapped(meta, is_active, t_pos, is_drum, font_name, font_size)
    ...
    if #tokens == 0 or not token_meta then
        return ""
    end

    local base_color = is_drum
            and (is_active and Options.drum_active_color or Options.drum_context_color)
        or (is_active and Options.srt_active_color or Options.srt_context_color)
    local opacity = calculate_ass_alpha(
        is_drum and (is_active and Options.drum_active_opacity or Options.drum_context_opacity)
    ...
    local bold_state = (
        is_active and (is_drum and Options.drum_active_bold or f_bold)
        or (is_drum and Options.drum_context_bold or f_bold)
    )
            and "1"
        or "0"
    local size = font_size
        * (is_active and Options.drum_active_size_mul or Options.drum_context_size_mul)
    ...
    for _, vl in ipairs(vlines) do
        ...
        for _, j in ipairs(vl.token_indices) do
            ...
            if meta_item.priority >= 1 or (meta_item.priority == 0 and meta_item.is_phrase) then
                local final_bold = (meta_item.priority == 3) and Options.anki_highlight_bold
                    or meta.h_bold
                local is_man = (meta_item.priority == 1 or meta_item.priority == 2)
                local bg_alpha = calculate_ass_alpha(
                    is_drum and Options.drum_bg_opacity or Options.srt_bg_opacity
                )
                table.insert(
                    formatted_parts,
                    format_highlighted_word(
                        ...
                        final_bold,
                        is_man,
                        is_drum and Options.drum_bg_color or Options.srt_bg_color,
                        bg_alpha,
                        is_drum and Options.drum_border_size or Options.srt_border_size
                    )
                )
            else
...
local function draw_drum(
    ...
    if view_center == -1 then
        return ""
    end

    if
        DRUM_DRAW_CACHE.subs_ptr == subs
        and DRUM_DRAW_CACHE.view_center == view_center
        and DRUM_DRAW_CACHE.active_idx == active_idx
        and DRUM_DRAW_CACHE.is_drum == (FSM.DRUM == "ON")
        and DRUM_DRAW_CACHE.highlight_count == #FSM.ANKI_HIGHLIGHTS
        and DRUM_DRAW_CACHE.layout_version == FSM.LAYOUT_VERSION
        and DRUM_DRAW_CACHE.al == FSM.DW_ANCHOR_LINE
        and DRUM_DRAW_CACHE.aw == FSM.DW_ANCHOR_WORD
        and DRUM_DRAW_CACHE.cl == FSM.DW_CURSOR_LINE
        and DRUM_DRAW_CACHE.cw == FSM.DW_CURSOR_WORD
        and DRUM_DRAW_CACHE.pending_version == (FSM.DW_CTRL_PENDING_VERSION or 0)
    then
        if hit_zones and DRUM_DRAW_CACHE.hit_zones then
            for k, v in ipairs(DRUM_DRAW_CACHE.hit_zones) do
                hit_zones[k] = v
            end
        end
    ...
    local font_name = is_drum_mode
            and (Options.drum_font_name ~= "" and Options.drum_font_name or mp.get_property(
                "sub-font",
                "Inter"
            ))
        or (
            Options.srt_font_name ~= "" and Options.srt_font_name
            or mp.get_property("sub-font", "Inter")
        )
    ...
    for i = start_idx, end_idx do
        ...
        local base_color = is_drum_mode
                and (is_active and Options.drum_active_color or Options.drum_context_color)
            or (is_active and Options.srt_active_color or Options.srt_context_color)

        local h_color = is_drum_mode
                and (is_pri and Options.drum_pri_highlight_color or Options.drum_sec_highlight_color)
            or (is_pri and Options.srt_pri_highlight_color or Options.srt_sec_highlight_color)
        local c_color = is_drum_mode
                and (is_pri and Options.drum_pri_ctrl_select_color or Options.drum_sec_ctrl_select_color)
            or (is_pri and Options.srt_pri_ctrl_select_color or Options.srt_sec_ctrl_select_color)
        local h_bold = is_drum_mode
                and (is_pri and Options.drum_pri_highlight_bold or Options.drum_sec_highlight_bold)
            or (is_pri and Options.srt_pri_highlight_bold or Options.srt_sec_highlight_bold)

        m.token_meta = populate_token_meta(
            subs,
            i,
            m.tokens,
            base_color,
    ...
    for i, m in ipairs(sub_metas) do
        local line_text = format_sub_wrapped(
            m,
            m.sub_idx == active_idx,
            subs[m.sub_idx].start_time,
            is_drum,
            font_name,
            font_size
        )
        if i == 1 then
            ...
            local line_fs = font_size
                * (prev_is_active and Options.drum_active_size_mul or Options.drum_context_size_mul)
    ...
    local style_block = string.format(
        "{\\bord%g}{\\shad%g}{\\3c&H%s&}{\\4c&H%s&}{\\3a&H%s&}{\\4a&H%s&}{\\q2}%s",
        bord,
        shad,
        bg_color,
        bg_color,
        calculate_ass_alpha(bg_opacity),
        calculate_ass_alpha(bg_opacity),
        vsp_tag
    )
    ...
    if hit_zones then
        ...
        for k, v in ipairs(hit_zones) do
            DRUM_DRAW_CACHE.hit_zones[k] = v
        end
    else
...
local function draw_dw(subs, view_center, active_idx)
    ...
    if
        ...
        and DW_DRAW_CACHE.active_idx == active_idx
        and DW_DRAW_CACHE.subs_ptr == subs
        and DW_DRAW_CACHE.highlight_count == #FSM.ANKI_HIGHLIGHTS
        and DW_DRAW_CACHE.layout_version == FSM.LAYOUT_VERSION
        and DW_DRAW_CACHE.cl == FSM.DW_CURSOR_LINE
        and DW_DRAW_CACHE.cw == FSM.DW_CURSOR_WORD
        and DW_DRAW_CACHE.al == FSM.DW_ANCHOR_LINE
        and DW_DRAW_CACHE.aw == FSM.DW_ANCHOR_WORD
        and DW_DRAW_CACHE.pending_version == (FSM.DW_CTRL_PENDING_VERSION or 0)
    then
    ...
    for layout_i, entry in ipairs(layout) do
        ...
        entry.token_meta = populate_token_meta(
            subs,
            i,
            entry.words,
            base_color,
            subs[i].start_time,
            entry,
            not Options.dw_pri_highlighting,
            Options.dw_highlight_color,
            Options.dw_ctrl_select_color
        )
    end
    ...
    for layout_i, entry in ipairs(layout) do
        ...
        local opacity = calculate_ass_alpha(
            is_active and Options.dw_active_opacity or Options.dw_context_opacity
        )
        local font_name = (Options.dw_font_name ~= "") and Options.dw_font_name
            or mp.get_property("sub-font", "Inter")
        local bold_state = (is_active and Options.dw_active_bold or Options.dw_context_bold) and "1"
            or "0"
        local f_size = Options.dw_font_size
            * (is_active and Options.dw_active_size_mul or Options.dw_context_size_mul)
        local line_prefix = string.format(
            "{\\fn%s}{\\fs%d}{\\b%s}{\\1c&H%s&}{\\1a&H%s&}",
            font_name,
            f_size,
            bold_state,
            color,
        ...
        for vl_index, vl_indices in ipairs(entry.vlines) do
            ...
            local line_style = string.format(
                "{\\pos(960, %g)}{\\an8}{\\bord%g}{\\shad%g}{\\3c&H%s&}{\\4c&H%s&}{\\3a&H%s&}{\\4a&H%s&}{\\q2}",
                vl_y_top,
                Options.dw_border_size,
                Options.dw_shadow_offset,
                Options.dw_bg_color,
                Options.dw_bg_color,
                bg_alpha,
                bg_alpha
...
local function draw_dw_tooltip(subs, target_line_idx, osd_y)
    local tooltip_sec_subs = (Tracks.sec.subs and #Tracks.sec.subs > 0) and Tracks.sec.subs
        or FSM.DW_TOOLTIP_SEC_SUBS
    if target_line_idx == -1 or not tooltip_sec_subs or #tooltip_sec_subs == 0 then
        return ""
    end

    if
        DW_TOOLTIP_DRAW_CACHE.target_idx == target_line_idx
        and DW_TOOLTIP_DRAW_CACHE.osd_y == osd_y
        and DW_TOOLTIP_DRAW_CACHE.version == FSM.LAYOUT_VERSION
        and DW_TOOLTIP_DRAW_CACHE.cl == FSM.DW_CURSOR_LINE
        and DW_TOOLTIP_DRAW_CACHE.cw == FSM.DW_CURSOR_WORD
        and DW_TOOLTIP_DRAW_CACHE.av == FSM.ANKI_VERSION
    then
        if DW_TOOLTIP_DRAW_CACHE.hit_zones then
            FSM.DW_TOOLTIP_HIT_ZONES = {}
            for k, v in ipairs(DW_TOOLTIP_DRAW_CACHE.hit_zones) do
                FSM.DW_TOOLTIP_HIT_ZONES[k] = v
            end
        end
    ...
    local font_name = (Options.tooltip_font_name ~= "") and Options.tooltip_font_name
        or mp.get_property("sub-font", "Inter")
    ...
    for i = start_idx, end_idx do
        ...
        local base_color = is_active and Options.tooltip_active_color
            or Options.tooltip_context_color
        local opacity = is_active and Options.tooltip_active_opacity
            or Options.tooltip_context_opacity
        local bold_state = (
            is_active and Options.tooltip_active_bold or Options.tooltip_context_bold
        )
                and "1"
            or "0"
        ...
        local token_meta = populate_token_meta(
            tooltip_sec_subs,
            i,
            tokens,
            base_color,
            ...
            nil,
            force_plain,
            Options.tooltip_highlight_color,
            Options.tooltip_ctrl_select_color
        )
        ...
        for _, indices in ipairs(vline_indices) do
            ...
            for _, idx in ipairs(indices) do
                ...
                local final_bold = (tm.priority == 3) and Options.anki_highlight_bold
                    or Options.tooltip_highlight_bold
    ...
    local total_gap =
        calculate_sub_gap("tooltip", fs, Options.tooltip_line_height_mul, Options.tooltip_vsp)
    ...
    for _, sm in ipairs(subtitle_metas) do
        for _, vl in ipairs(sm.visual_lines) do
            ...
            local line_ass =
                format_tooltip_text_event(style_ctx, anchor_x, line_center_y, vl.line_text)
    ...
    local bg_rect =
        format_tooltip_card_event(style_ctx, rect_left, rect_top, rect_w, rect_h, rect_bg_alpha)
    ...
    for k, v in ipairs(FSM.DW_TOOLTIP_HIT_ZONES) do
        DW_TOOLTIP_DRAW_CACHE.hit_zones[k] = v
    end
