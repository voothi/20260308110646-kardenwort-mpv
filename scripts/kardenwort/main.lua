...
local function draw_dw(subs, view_center, active_idx)
    ...
    -- Text Block mapping
    -- Collect per-visual-line metadata for unified block rendering.
    -- Hit-zones are populated from the same y_top coordinates so render ≡ hit-test.
    local visual_lines = {}
    for layout_i, entry in ipairs(layout) do
        local i = entry.sub_idx
    ...
    -- Build unified block text with per-visual-line \vsp deltas.
    -- vsp_val = (y_top[k] - y_top[k-1]) - font_size[k-1]
    -- This guarantees render spacing is byte-identical to hit-zone y-coordinates.
    local block_text = ""
    ...
    -- \an5 renders the entire multi-line block as a single cohesive background box.
    -- Position translates block_top to the vertical center for \an5 anchoring.
