...
local function draw_dw(subs, view_center, active_idx)
    ...
    local final_ass = string.format("{\\pos(960, %g)}{\\an8}{\\bord%g}{\\shad%g}{\\3c&H%s&}{\\4c&H%s&}{\\3a&H%s&}{\\4a&H%s&}{\\q2}{\\fs%d}%s%s", 
...
local function draw_dw_tooltip(subs, target_line_idx, osd_y)
    ...
    for _, sm in ipairs(subtitle_metas) do
        for _, vl in ipairs(sm.visual_lines) do
            local line_x_start
            if dm_mode then
                line_x_start = (base_w / 2) - (vl.width / 2)
            else
                line_x_start = anchor_x - vl.width
            end
    ...
    for _, sm in ipairs(subtitle_metas) do
        ...
        for _, vl in ipairs(sm.visual_lines) do
            table.insert(sub_visual_lines, vl.line_text)
        end
    ...
    local ass = string.format("{\\fn%s}%s{\\pos(%g, %g)}{\\an%d}{\\fs%d}{\\b%s}{\\bord%g}{\\shad%g}{\\3c&H%s&}{\\4c&H%s&}{\\3a&H%s&}{\\4a&H%s&}{\\q2}%s",
        font_name, vsp_tag, posX, posY, alignment, fs, base_bold, bord, shad, bg_color, bg_color, bg_alpha, bg_alpha, text_block)
