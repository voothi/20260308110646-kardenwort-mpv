...
local FSM = {
    ...
    DW_PROTECTED_SELECTION = false,  -- Locks cursor overwrite for clicks inside selection
        ...
        if tbl.event == "down" then
            ...
            if line_idx then
                ...
                if word_idx and updates_selection then
                    ...
                    FSM.DW_PROTECTED_SELECTION = is_inside
...
-- Register Bindings
mp.add_key_binding("s", "toggle-autopause", cmd_toggle_autopause)
mp.add_key_binding("q", "toggle-karaoke-mode", cmd_toggle_karaoke)
mp.add_key_binding("SPACE", "smart-space", cmd_smart_space, {complex=true})
mp.add_key_binding("c", "toggle-drum-mode", cmd_toggle_drum)
mp.add_key_binding("v", "toggle-sub-visibility", cmd_toggle_sub_vis)
mp.add_key_binding("R", "cycle-secondary-pos", cmd_cycle_sec_pos)
mp.add_key_binding("E", "cycle-sec-sid", cmd_cycle_sec_sid)
mp.add_key_binding("V", "toggle-osc-visibility", cmd_toggle_osc)
mp.add_key_binding("Ctrl+c", "copy-subtitle", cmd_copy_sub)
mp.add_key_binding("m", "cycle-copy-mode", cmd_cycle_copy_mode)
mp.add_key_binding("M", "toggle-copy-context", cmd_toggle_copy_ctx)
mp.add_key_binding("w", "toggle-drum-window", cmd_toggle_drum_window)
mp.add_key_binding("f", "toggle-drum-search", cmd_toggle_search)
mp.add_key_binding("b", "toggle-book-mode", toggle_book_mode)
mp.add_forced_key_binding("b", "book-mode-b", toggle_book_mode)
mp.add_forced_key_binding("и", "book-mode-ru", toggle_book_mode)
mp.add_key_binding("a", "lls-seek_prev", function(t) cmd_seek_with_repeat(-1, t) end, {complex = true})
mp.add_key_binding("d", "lls-seek_next", function(t) cmd_seek_with_repeat(1, t) end, {complex = true})
mp.add_key_binding("g", "toggle-anki-global", cmd_toggle_anki_global)
