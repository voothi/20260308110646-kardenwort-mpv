...
local function format_highlighted_word(word, h_color, base_color, is_phrase, bold_state, use_1c, force_bold, force_full)
    ...
    if is_phrase or force_full then
        -- Full highlighting for phrases or manual selections (Gold/Pink)
        return string.format("%s{\\%s&H%s&}%s{\\%s&H%s&}%s", b_on, c_tag, h_color, word, c_tag, base_color, b_off)
    else
        -- Surgical highlighting for single database words (professional look: punctuation uncolored)
        ...
        if mid ~= "" then
            ...
            -- If the word is ONLY punctuation, and we aren't forcing full (automated match),
            -- we keep it uncolored for the "Premium" look.
            return word
        end
    end
end
        ...
        for _, vl_indices in ipairs(entry.vlines) do
            ...
            for _, j in ipairs(vl_indices) do
                ...
                if meta_item.priority >= 1 or (meta_item.priority == 0 and meta_item.is_phrase) then
                    ...
                    local force_full = (meta_item.priority == 1 or meta_item.priority == 2)
                    table.insert(formatted_words, format_highlighted_word({text = meta_item.text}, meta_item.color, color, meta_item.is_phrase, bold_state, true, final_bold, force_full))
                else
