# Spec: Configurable Replay Feedback

## Purpose
Users should have full control over the visual feedback provided during replay operations to suit their learning style and immersion preferences.
...
## Verification
- Verify that `%m`, `%s`, `%c`, and `%x` are correctly substituted in OSD messages.
- Confirm that default templates are used if no overrides exist in `mpv.conf`.
- Verify that `%x` logic correctly distinguishes between single and multiple iterations.
