...
- **WHEN** `master_tick` cakardenwort `tick_drum` and the `center_idx`, selection state (anchor/cursor line+word), `DW_CTRL_PENDING_VERSION`, and highlight count have not changed since the last call
