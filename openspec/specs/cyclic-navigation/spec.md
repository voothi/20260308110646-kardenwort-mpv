# cyclic-navigation Specification

## Purpose
TBD - created by archiving change 20260505150404-cyclic-subtitle-nav. Update Purpose after archive.
## Requirements
...
- **WHEN** the playhead is at the first subtitle (index 1) and the `kardenwort-seek_prev` command is executed
- **THEN** the system SHALL seek to the last subtitle in the track

#### Scenario: Wrap-around from last to first
- **WHEN** the playhead is at the last subtitle and the `kardenwort-seek_next` command is executed
