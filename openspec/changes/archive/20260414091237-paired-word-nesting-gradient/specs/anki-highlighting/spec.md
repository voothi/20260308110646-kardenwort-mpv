...
#### Scenario: Alpha gradient applied based on split term nesting
- **WHEN** a split multi-word term overlaps with another highlighted term (either split or contiguous)
- **THEN** the system SHALL calculate its nesting depth
- **AND** apply an alpha channel modifier so that overlapping segments are visually distinct.
