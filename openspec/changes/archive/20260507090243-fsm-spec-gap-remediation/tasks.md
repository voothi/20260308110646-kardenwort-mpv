...
- [x] 1.3 Manual smoke test: open DW, press 's', confirm no crash and DW renders unchanged; close DW, confirm subtitles are in the toggled state.
...
- [x] 2.3 Manual smoke test with dual SRT tracks loaded: scrub playback, confirm primary track focus sentinel behaves as before; confirm secondary track does not snap incorrectly on long audio-padding values.
...
- [x] 3.4 Manual smoke test: adjust secondary pos via delta key, enter Drum Mode, press cycle-secondary-pos — confirm toggle direction matches current visual position.

## 4. Spec Verification

- [x] 4.1 Confirm `openspec/specs/fsm-architecture/spec.md` is updated with the three MODIFIED requirements from this change (run `openspec archive` or verify manually).
