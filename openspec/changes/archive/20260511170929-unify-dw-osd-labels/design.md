...
1. **Unify "Managed by" Feedback**: Add `if FSM.DRUM_WINDOW ~= "OFF" then show_osd("Managed by Drum Window") return end` to:
...
- **OSD Spam**: Users might find the "Managed by" message annoying if they repeatedly press keys, but this is consistent with existing `x` key behavior and serves as a clear indicator of WHY a key isn't doing its default action.
