...
### Requirement: Drum Mode Result Cache
The `draw_drum()` function SHALL maintain a result cache that stores the last generated ASS string and the state used to generate it. If the current state matches the cached state, the function SHALL return the cached string without rebuilding.
