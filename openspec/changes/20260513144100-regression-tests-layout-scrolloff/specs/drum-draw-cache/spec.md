...
### Requirement: Cache Shape Verification
The layout cache SHALL only be used if all required metadata fields (e.g., `height`, `tokens`) are present in the cached entries.

#### Scenario: Missing height metadata in cache
- **WHEN** `dw_build_layout` encounters a cached entry without a `height` field
- **THEN** the system SHALL treat the cache as invalid for that entry
- **AND** the system SHALL rebuild the full layout metadata for that line to avoid `nil` arithmetic crashes.
