...
The `block_height` calculation in `draw_dw_tooltip` will be updated to sum the heights of all *visual* (wrapped) lines across all logical subtitle blocks, plus the appropriate inter-subtitle gaps (`calculate_sub_gap`). To maintain precise Y-axis alignment with the primary Drum Window, empty or metadata-only subtitles MUST NOT be skipped; instead, they will contribute exactly one logical line height to the total block height. This ensures that the vertical centering (`final_y`) and clamping logic correctly accounts for multi-line and empty subtitles, maintaining perfect parity with the main Drum Window hit-testing zones.
