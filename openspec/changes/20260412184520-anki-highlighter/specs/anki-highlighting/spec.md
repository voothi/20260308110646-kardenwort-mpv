...
- **THEN** the system extracts the literal selected string and a broadened context window (including surrounding subtitles), and appends/updates an Anki-compatible TSV row.
...
- **THEN** the rendering engine swaps between evaluating the TSV terms globally vs locally.
...
- **THEN** within a configurable interval, the player system reloads the file atomically (using `pcall` for safety) and refreshes all active subtitle viewports.
