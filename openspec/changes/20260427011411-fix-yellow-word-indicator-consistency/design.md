...
3. **Retain Sticky-X and Navigation Logic**: The existing `cmd_dw_line_move` and `cmd_dw_word_move` logic already handles "middle of line" and "start/end of line" when the cursor is fresh (thanks to `FSM.DW_CURSOR_X = nil` and `FSM.DW_CURSOR_WORD = -1` being set by `Esc`). The fix for stale lines will automatically make these work correctly from the current playback position.
