...
- **Multi-Occurrence Persistence**: Move `ANKI_HIGHLIGHTS` from a map to a list structure. This ensures that saving a word in a new location preserved the highlighting in previous locations (fixing the temporal window conflict).
- **Overlap-Only Intensity**: Highlighting depth (color stacking) SHALL be calculated based on unique term strings. Redundant bookmarks for the exact same term across different times SHALL NOT increase intensity; only distinct overlapping phrases (e.g., a single word vs. a larger phrase) trigger darker colors.
