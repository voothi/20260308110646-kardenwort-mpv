...
The current `lls_core.lua` script (Post-v1.44.2 HEAD) implements a highly refined interaction and grounding engine. This design document formalizes the architectural decisions that govern Multi-Pivot Grounding, persistent selection sets, and the new **Precision Neighborhood Verification** system.
...
- **Match Requirement**: A highlight is only rendered if at least one meaningful word (length >= 2, stripped of punctuation) from the neighborhood matches the stored `data.__ctx_lower` (context).
...
- **Shield**: 150ms temporal lock (`FSM.DW_MOUSE_LOCK_UNTIL`) to filter hardware jitter.
...
- **UPSR**: Codifies spacing rules for punctuation (e.g., no space before/after `/`, `-`).
- **Ellipses**: Injects ` ... ` into `ctrl_pending_set` terms when gaps are detected (Adaptive Gap Detection).

## Risks / Trade-offs

- **Recall vs. Precision**: The 1-word intersection threshold is low to favor learners, but significantly improves precision over legacy literal matching.
