...
- [ ] 1.2 Update `tests/ipc/mpv_ipc.py` to replace `lls_core` with `kardenwort` in all `script-message-to` calls.
- [ ] 1.3 Perform a global search and replace in the `tests/` directory for `lls_core` -> `kardenwort`.

## 2. Filesystem Restructuring

- [ ] 2.1 Create directory `scripts/kardenwort/`.
- [ ] 2.2 Move `scripts/lls_core.lua` to `scripts/kardenwort/main.lua`.
- [ ] 2.3 Move `scripts/lls_utils.lua` to `scripts/kardenwort/utils.lua`.
...
- [ ] 3.2 Update `main.lua` to replace all `[LLS]` log prefixes with `[Kardenwort]`.
- [ ] 3.3 Update `main.lua` to replace `user-data/lls/` with `user-data/kardenwort/`.
