...
- [x] 2.1 Implement `vline_height(fs)` helper in a scoped helper block/table.
...
- [x] 2.4 Visual gate: verify wrap spacing, inter-subtitle gap consistency, and safe margins at viewport extremes.
- [x] 2.5 Hit-accuracy calibration pass (do immediately after 2.4): verify DW and tooltip pointer-to-word mapping at top/middle/bottom lines and tune geometric offsets only if drift is reproducible.

## 3. Phase 3: Border-Style Lifecycle Hardening (Foundational)

- [x] 3.1 Implement `manage_ui_border_override(enable)` to switch `osd-border-style` only while custom overlays are active.
- [x] 3.2 Add nested depth safety (`ui_border_override_depth`) to prevent premature restoration under overlapping menus.
- [x] 3.3 Validate open-close lifecycle across DW/Search/Tooltip transitions.
...
- [x] 4.1 Calibrate Cyrillic width in `dw_get_str_width()` using `dw_cyrillic_coef`.
- [x] 4.2 Center DM tooltip text/card at `X = 960` with `\an8`.
- [x] 4.3 Re-align `dw_tooltip_hit_test` bounds to centered geometry (`960 - line_w / 2`).
- [x] 4.4 Place DM tooltip vertically in the secondary subtitle region to avoid collisions.
...
- [x] 5.1 Implement unified tooltip vector card (`\p1`) first, preserving current interaction flow.
- [x] 5.2 Implement unified DW vector card (`\p1`) second, replacing line-by-line frame visuals.
- [x] 5.3 Add local compatibility tags (`\3a&HFF&`, `\4a&HFF&`, `\bord0`, `\shad0`) for `background-box` mode stability.
- [x] 5.4 Keep existing color constants and text-style tags unchanged unless a specific bug forces a local override.
...
- [x] 6.1 Add trailing punctuation selection parity (`.`, `?`, `!`) by relaxing the `t.is_word` filter in `calculate_osd_line_meta()`.
- [x] 6.2 Reassess residual drift after Phase 2.5 and Phase 4.5; only then consider `FSM.DW_HIT_ZONES` cache + `dw_hit_test()` rewrite.
...
- [x] 7.1 Explicitly avoid global startup callback shims.
- [x] 7.2 If runtime safety issues remain, add narrow local guards at proven unstable call sites only.
- [x] 7.3 Keep `manage_dw_bindings()` architecture unchanged unless a concrete regression demands a targeted fix.
