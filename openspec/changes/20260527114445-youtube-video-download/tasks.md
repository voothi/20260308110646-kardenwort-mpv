...
- [x] 15.2a For `sync_secondary_srt_timestamps`, build an **ordered** `subtitles_all_present` list (do not use a set), preserving `sub_langs_list` order while including both pre-existing and newly written files; gate sync on `len(subtitles_all_present) >= 2` so a freshly-downloaded secondary is still aligned against a pre-existing primary and primary-track selection remains deterministic
