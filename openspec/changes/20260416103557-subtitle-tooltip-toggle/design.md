...
- **Off-Screen Suppression:** To prevent "blinking" artifacts, the keyboard-toggled tooltip ('e') SHALL only be rendered if its target subtitle line is currently visible within the Drum Window OSD. If the focus (active line or cursor) is scrolled off-screen, the toggle action will set the state but the OSD will remain empty until the line scrolls back into view.
...
- **Book Mode Cursor Decoupling:** In Book Mode ON, the manual selection cursor (yellow) is explicitly decoupled from the active playback subtitle (white).
- **Persistent Manual Selection:** To ensure consistency across all modes, manual selections (including single-word clicks) SHALL persist during navigation (e.g., via `a`/`d`). In Book Mode OFF, the yellow focus SHALL NOT reset to the active playback line if a manual selection anchor is present. This prevents "jumping" when reviewing translations while seeking.
