...
- **WHEN** the user interacts with a word that is part of a pending Paired Selection Set (Pink) using a "Smart Add" trigger (configured in `dw_key_add`)
...
- **THEN** it SHALL be rendered in the paired choice color regardless of any active contiguous selection (Yellow/Red) or existing database highlights (Orange/Purple).
