...
- [x] 2.4 Return the constructed ASS string. Anchor it to the right: `{\pos(1850, osd_y)}{\an6}`. Use `{\1a&H...&}{\1c&H...&}{\p1}...{\p0}` for a background rectangle and properly style the text. Ensure text wraps correctly or enforce `\q1`.

## 3. Tooltip Interaction Handlers

- [x] 3.1 Write function `cmd_dw_tooltip_pin()`. Use `dw_get_mouse_osd()` and `dw_hit_test(osd_x, osd_y)`. If a `line_idx` is hit, set `FSM.DW_TOOLTIP_LINE = line_idx` and manually call `draw_dw_tooltip(...)` updating `dw_tooltip_osd.data`.
...
- [x] 4.2 In `manage_dw_bindings()`, inside the local `keys` table, add a new binding: `{key = "MBTN_RIGHT", name = "dw-tooltip-pin", fn = cmd_dw_tooltip_pin}`.
