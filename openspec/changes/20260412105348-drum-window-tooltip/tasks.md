...
- [ ] 3.3 Add conditional Hook for Hover Mode: If configured to "HOVER", auto-assign `FSM.DW_TOOLTIP_LINE` without requiring the `MBTN_RIGHT` click.
- [ ] 3.4 Wire `dw_tooltip_osd:update()` to fire when `FSM.DW_TOOLTIP_LINE` effectively toggles or drops.
