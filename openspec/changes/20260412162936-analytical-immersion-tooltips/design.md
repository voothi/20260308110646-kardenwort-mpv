...
Update `make_mouse_handler` to set the suppression state on `down` events.

**Process**:
1. On `down`: Clear active tooltip OSD and set `FSM.DW_TOOLTIP_LOCKED_LINE` to the current hit-tested line index.
2. On `up`: Keep `FSM.DW_TOOLTIP_LOCKED_LINE` intact.
...
- IF `FSM.DW_MOUSE_DRAGGING` is true OR `line_idx == FSM.DW_TOOLTIP_LOCKED_LINE`:
    - Force tooltip OSD data to empty.
    - Reset `FSM.DW_TOOLTIP_LINE` (to trigger a redraw when lock is eventually released).
