...
def _query_tooltip_state(ipc):
    ipc.command(["script-message", "kardenwort-test-query-tooltip-state"])
    time.sleep(0.2)
    raw = ipc.get_property("user-data/kardenwort-test-tooltip-state")
    return json.loads(raw) if raw else {}


def _assert_stable_hold_payload(ipc, x, y, samples=5):
    ipc.command(["script-message", "kardenwort-test-dw-tooltip-pin-at", str(x), str(y), '{"event":"down"}'])
    time.sleep(0.5)
    ...
    ipc.command(["script-message", "kardenwort-test-dw-tooltip-pin-at", str(x), str(y), '{"event":"up"}'])
    time.sleep(0.1)
...
def test_20260511102747_tooltip_stability_fixed_pos(mpv_fragment2):
    ...
    ipc.command(["script-message", "kardenwort-test-dw-toggle"])
    ...
    ipc.command(["script-message", "kardenwort-test-hit-test", "960", "540"])
    time.sleep(0.5)
    
    # Start holding RMB at center
    ipc.command(["script-message", "kardenwort-test-dw-tooltip-pin", '{"event":"down"}'])
    time.sleep(0.5)
    ...
    def get_tooltip_state():
        ...
        time.sleep(0.3)
        try:
            val = ipc.get_property("user-data/kardenwort-test-tooltip-state")
    ...
    ipc.command(["script-message", "kardenwort-test-query-tooltip-state"])
    time.sleep(0.2)
    state2 = json.loads(ipc.get_property("user-data/kardenwort-test-tooltip-state"))
    
    # The data should be identical
    assert state2["data"] == state1["data"], "Tooltip data changed while mouse was still!"
    
    # Release RMB
    ipc.command(["script-message", "kardenwort-test-dw-tooltip-pin", '{"event":"up"}'])
    time.sleep(0.1)
    ipc.command(["script-message", "kardenwort-test-query-tooltip-state"])
    state3 = json.loads(ipc.get_property("user-data/kardenwort-test-tooltip-state"))
...
def test_20260511102747_y_rounding_stability(mpv_fragment2):
    ...
    for _ in range(5):
        ipc.command(["script-message", "kardenwort-test-query-tooltip-state"])
        time.sleep(0.2)
        state = json.loads(ipc.get_property("user-data/kardenwort-test-tooltip-state"))
...
def _pick_hit_zone_center(ipc):
    ipc.command(["script-message", "kardenwort-test-query-hit-zones"])
    time.sleep(0.2)
