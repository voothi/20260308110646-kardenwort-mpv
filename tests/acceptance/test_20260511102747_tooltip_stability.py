...
def _query_tooltip_state(ipc):
    ...
    time.sleep(0.2)
    raw = ipc.get_property("user-data/test-tooltip-state")
...
def _assert_stable_hold_payload(ipc, x, y, samples=5):
    ...
    ipc.command(["script-message-to", "kardenwort", "test-dw-tooltip-pin-at", str(x), str(y), '{"event":"up"}'])
    time.sleep(0.1)
...
def test_20260511102747_tooltip_stability_fixed_pos(mpv_fragment2):
    ...
    ipc.command(["script-message-to", "kardenwort", "test-hit-test", "960", "540"])
    time.sleep(0.5)
    
    # Start holding RMB at center
    ipc.command(["script-message-to", "kardenwort", "test-dw-tooltip-pin", '{"event":"down"}'])
    time.sleep(0.5)
    ...
    ipc.command(["script-message-to", "kardenwort", "test-query-tooltip-state"])
    time.sleep(0.2)
    state2 = json.loads(ipc.get_property("user-data/test-tooltip-state"))
...
def test_20260511102747_y_rounding_stability(mpv_fragment2):
    ...
    for _ in range(5):
        ipc.command(["script-message-to", "kardenwort", "test-query-tooltip-state"])
        time.sleep(0.2)
        state = json.loads(ipc.get_property("user-data/test-tooltip-state"))
