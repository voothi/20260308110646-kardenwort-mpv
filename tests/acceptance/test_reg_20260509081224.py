...
class TestOpenSpecComplianceBatch2:
    ...
    def test_reg_lua_options_fallback_defaults(self, mpv):
    ...
    def test_reg_lua_scoping_correction_calls(self, mpv):
        ...
        ipc.command(['script-message-to', 'lls_core', 'lls-test-get-tokens', 'Test boundary'])
        time.sleep(0.2)
        state = robust_query_state(ipc)
        assert 'test_data' in state
        assert 'test_tokens' in state['test_data']

    def test_reg_metadata_tag_filtering_strip(self, mpv):
        """Verify [musik] tags are stripped from context but preserved in selection (metadata-tag-filtering)."""
        ipc = mpv.ipc
        ipc.command(['script-message-to', 'lls_core', 'lls-test-set-option', 'anki_strip_metadata', 'yes'])
        time.sleep(0.2)
        
        # Test case: [musik] Die Luftfahrt
        ipc.command(['script-message-to', 'lls_core', 'lls-test-get-tokens', '[musik] Die Luftfahrt'])
        time.sleep(0.2)
        state = robust_query_state(ipc)
    ...
    def test_reg_mmb_drag_export_hotkey(self, mpv):
    ...
    def test_reg_mode_based_calibration_v34(self, mpv):
    ...
    def test_reg_modular_architecture_loading(self, mpv):
        """Verify that the script didn't crash, implying modules loaded (modular-architecture)."""
        state = robust_query_state(mpv.ipc)
        assert state is not None
        assert 'dw_cursor' in state

    def test_reg_multi_dimensional_relevance_scoring_constants(self, mpv):
        ...
        ipc = mpv.ipc
        ipc.command(['script-message-to', 'lls_core', 'lls-test-search-input', 'test'])
        time.sleep(0.2)
        state = robust_query_state(ipc)
        assert state is not None

    def test_reg_multi_line_substring_selection_aggregation(self, mpv):
        """Verify Shift-navigation for range selection (multi-line-substring-selection)."""
        ipc = mpv.ipc
        # Ensure we have a sub active
        ipc.command(['script-message-to', 'lls_core', 'lls-test-dw-double-click', '1'])
        time.sleep(0.5)
        ...
        time.sleep(0.2)
        ipc.command(['script-message-to', 'lls_core', 'lls-test-dw-key', 'Shift+RIGHT'])
        time.sleep(0.5)
    ...
    def test_reg_native_conflict_management_suppression(self, mpv):
    ...
    def test_reg_optimized_defaults_v34_values(self, mpv):
    ...
    def test_reg_phrase_trailing_punctuation_boundaries(self, mpv):
        """Verify strict selection boundaries (phrase-trailing-punctuation)."""
        # This is a behavior test for export preparation
        pass

    def test_reg_platform_detection_logic_windows(self, mpv):
    ...
    def test_reg_platform_utility_mapping_retries(self, mpv):
