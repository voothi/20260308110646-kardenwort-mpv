...
class TestHistoricalRegressionsV2:
    ...
    def test_drum_window_indexing_simple(self, mpv):
        ...
        ipc.command(['script-message-to', 'kardenwort', 'test-get-tokens', 'Sie hören die Nachrichtensendung nur einmal.'])
        time.sleep(0.5)
    ...
    def test_drum_window_performance_cache_invalidation(self, mpv):
        ...
        # Change font size
        ipc.command(['script-message-to', 'kardenwort', 'test-set-option', 'dw_font_size', '45'])
        time.sleep(0.5)
    ...
    def test_drum_window_sticky_navigation(self, mpv):
        ...
        ipc.command(['script-message-to', 'kardenwort', 'test-dw-key', 'RIGHT'])
        time.sleep(0.2)
        state = robust_query_state(ipc)
    ...
    def test_drum_window_tooltip_toggle(self, mpv):
        ...
        # Toggle tooltip
        ipc.command(['script-message-to', 'kardenwort', 'test-dw-key', 'e'])
        time.sleep(0.5)
    ...
    def test_dw_mouse_selection_engine_double_click(self, mpv):
        ...
        # Double click line 2
        ipc.command(['script-message-to', 'kardenwort', 'test-dw-double-click', '2'])
        time.sleep(1.0)
    ...
    def test_dynamic_contrast_rendering_truncation(self, mpv):
        ...
        ipc = mpv.ipc
        long_str = "A" * 150
        ipc.command(['script-message-to', 'kardenwort', 'test-truncate', long_str])
        time.sleep(0.5)
        state = robust_query_state(ipc)
        test_data = state.get('test_data', {})
    ...
    def test_export_engine_hardening_brackets(self, mpv):
        ...
        ipc.command(['script-message-to', 'kardenwort', 'test-validate-term', '[...]'])
        time.sleep(0.5)
        state = robust_query_state(ipc)
        test_data = state.get('test_data', {})
    ...
    def test_fuzzy_search_optimization_match(self, mpv):
        ...
        ipc = mpv.ipc
        ipc.command(['script-message-to', 'kardenwort', 'test-fuzzy-match', 'hl wrd', 'hello world'])
        time.sleep(0.5)
        state = robust_query_state(ipc)
        test_data = state.get('test_data', {})
    ...
    def test_global_navigation_bindings(self, mpv):
        """Verify seek_next binding (global-navigation-bindings)."""
        ipc = mpv.ipc
    ...
    def test_global_semantic_coloring_brackets(self, mpv):
        ...
        ipc = mpv.ipc
        ipc.command(['script-message-to', 'kardenwort', 'test-get-tokens', '[UMGEBUNG]'])
        time.sleep(0.5)
        state = robust_query_state(ipc)
        test_data = state.get('test_data', {})
        tokens = test_data.get('test_tokens', [])
